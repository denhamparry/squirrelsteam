...
const scoreBreakdown = z.object({ us: score, them: score });
...
const fixtures = defineCollection({
  ...
  schema: z
    .object({
      ...
      result: z
        .object({
          us: score,
          them: score,
          tries: scoreBreakdown.optional(),
          conversions: scoreBreakdown.optional(),
        })
        .optional(),
    ...
    .refine(({ type, result }) => !result || type === "match", {
      message: "Results can only be recorded for match fixtures",
      path: ["result"],
    }),
});
